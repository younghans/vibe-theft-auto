// Generated from Natural Earth 1:50m admin-0 country data for the geography globe minigame.

export const SCHOOL_GEOGRAPHY_TARGET_COUNTRIES = Object.freeze([
  {
  "id": "afg",
  "name": "Afghanistan",
  "lat": 34.1643,
...
export function normalizeSchoolGeographyCountryName(value = '') {
  return String(value ?? '')
    ...
    .toLowerCase()
    .replace(/&/gu, ' and ')
    .replace(/\bu\s*s\s*a\b/gu, 'usa')
    .replace(/\bu\s*k\b/gu, 'uk')
    .replace(/\bst\b/gu, 'saint')
    .replace(/[^a-z0-9]+/gu, ' ')
    .replace(/\bthe\b/gu, ' ')
    .replace(/\s+/gu, ' ')
    .trim();
...
export function getSchoolGeographyCountryAliases(country = null) {
  if (!country) {
    return [];
  }
  const aliases = [];
...
export function isSchoolGeographyCountryAnswer(country = null, guess = '') {
  const normalizedGuess = normalizeSchoolGeographyCountryName(guess);
  if (!normalizedGuess) {
    return false;
  }
