...
const gameSource = await fsp.readFile(new URL('../src/game/Game.js', import.meta.url), 'utf8');
...
try {
  ...
  assert.match(hudSource, /data-admin-prompt-action-pending/, 'HUD prompt action buttons should expose pending action state.');
  assert.match(hudSource, /createAdminPromptActionButton/, 'HUD should render prompt action buttons through a shared pending-state helper.');
  assert.match(hudSource, /getAdminPromptPendingStatusText/, 'HUD should show a prompt action status while requests are in flight.');
  assert.match(hudSource, /Approving\.\.\./, 'HUD should show an approving deploy loading label.');
  assert.match(hudSource, /Sending\.\.\./, 'HUD should show a follow-up loading label.');
  assert.match(hudSource, /aria-busy="true"/, 'HUD pending prompt action buttons should report busy state.');
  assert.match(gameSource, /adminPromptPendingAction/, 'Game should track prompt action requests separately from global submit state.');
  assert.match(gameSource, /setAdminPromptPendingAction\('approve-deploy'/, 'Game should set pending state while approving deploys.');
  assert.match(gameSource, /mergeAdminPromptTask\(result\.task\)/, 'Game should merge returned task state before refreshing prompt tasks.');
