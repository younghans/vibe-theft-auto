...
function validateBartenderFunction() {
  ...
  assert(
    /SKATEBOARD_LOWER_BODY_STILL_BONES\s*=\s*Object\.freeze\(\[\.\.\.LOWER_BODY_LOCOMOTION_BONES\]\)/.test(playerSource)
      ...
      && /SKATEBOARD_REVERSED_LOWER_BODY_YAW\s*=\s*Math\.PI/.test(playerSource)
      && /\[MIXAMO_BONES\.hips\]:\s*Object\.freeze\(\[0,\s*SKATEBOARD_REVERSED_LOWER_BODY_YAW,\s*0\]\)/.test(playerSource)
      && /\[MIXAMO_BONES\.spine\]:\s*Object\.freeze\(\[0,\s*-SKATEBOARD_REVERSED_LOWER_BODY_YAW,\s*0\]\)/.test(playerSource)
    ...
    'Skating should hold the full player body still with the lower body rotated 180 degrees and both feet perpendicular to the skateboard'
  );
  assert(
